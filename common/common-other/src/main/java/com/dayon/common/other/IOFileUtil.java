...
public class IOFileUtil {
	...
	public static void copyFile(String pathName, File cpFile, boolean isCover) throws Exception {
		if (pathName == null || cpFile == null || cpFile.length() == 0) {
			throw new Exception("the pathName or downLoadFile is empty");
		}
		File file = new File(pathName);
		if (file.isDirectory() || file.isHidden()) {
			throw new Exception("lack power,the pathName exist same directory or hiddenFile");
		}
		if (file.isFile() && !isCover) {
			if (!isCover) {
				throw new Exception("the pathName exist same file and isCover is false");
			} else {
				file.delete();
			}
	...
	public static void writeFile(String filePath, byte[] bytes, boolean isCover) throws Exception {
		if (filePath == null || bytes == null || bytes.length == 0) {
			throw new Exception("the filePath or bytes is empty");
		}
		File file = new File(filePath);
		if (file.isDirectory() || file.isHidden()) {
			throw new Exception("lack power,the filePath exist same directory or hiddenFile");
		}
		if (file.isFile() && !isCover) {
			throw new Exception("the filePath exist same file and isCover is false");
		}
		FileOutputStream fos = null;
		try {
