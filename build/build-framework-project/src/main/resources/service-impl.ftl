...
public class ${serviceImplTypeInfo.simpleName} implements ${serviceTypeInfo.simpleName}{
	...
	@Override
	public PageDataResult<${entityTypeInfo.simpleName}> pageFind(DataMap paramMap, Integer page, Integer limit){
		try {
			...
			long count=${mapperTypeInfo.javaName}.count(paramMap);
			return new PageDataResult<>("查询成功",${entityTypeInfo.javaName}s, new Paging(page, limit, count));
		} catch (Exception e) {
			logger.error(e.getMessage(),e);
			return new PageDataResult<>(-1,"未知异常");
		}
	}
	@Override
