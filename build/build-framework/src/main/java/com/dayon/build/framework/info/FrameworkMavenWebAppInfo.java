...
public class FrameworkMavenWebAppInfo implements MavenProjectBuildInfo {
	...
	public FrameworkMavenWebAppInfo() {
	...
	public Map<String, File> getResourcesFileMap() {
		...
		File file = new File(Thread.currentThread().getContextClassLoader()
				.getResource("copy/web/framework-web.properties").getPath());
		resourcesFileMap.put("src/main/resources/framework-web.properties", file);

		file = new File(Thread.currentThread().getContextClassLoader().getResource("copy/web/log4j2.xml").getPath());
		resourcesFileMap.put("src/main/resources/log4j2.xml", file);


		file = new File(
				Thread.currentThread().getContextClassLoader().getResource("copy/web/spring-dubbo.xml").getPath());
		resourcesFileMap.put("src/main/resources/spring-dubbo.xml", file);

		file = new File(
				Thread.currentThread().getContextClassLoader().getResource("copy/web/spring-mvc.xml").getPath());
		resourcesFileMap.put("src/main/resources/spring-mvc.xml", file);
		file = new File(
				Thread.currentThread().getContextClassLoader().getResource("copy/web/favicon.ico").getPath());
		resourcesFileMap.put("src/main/webapp/favicon.ico", file);
		
		file = new File(
				Thread.currentThread().getContextClassLoader().getResource("copy/web/static").getPath());
		resourcesFileMap.put("src/main/webapp/static", file);
